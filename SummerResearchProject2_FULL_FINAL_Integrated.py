...
def index():
    if request.method == 'POST':
        ...
        # ----------- Begin: Injected Flight + Hotel Cost Logic -----------
        flight_options, hotel_options = [], []
        cheapest_flight, cheapest_hotel = None, None
        total_travel_cost = 0

        try:
            # Fetch Flights from JFK to SYD
            flight_resp = requests.get("https://api.flightapi.io/onewaytrip/6855816c8a2aa0fa24ef7369/JFK/SYD/2025-12-15/1/0/0/Economy/USD")
            flights = flight_resp.json().get("data", [])

            for f in flights:
                option = {
                    'airline': f.get("itineraries", [{}])[0].get("segments", [{}])[0].get("carrierCode", ""),
                    'from': f.get("origin"),
                    'to': f.get("destination"),
                    'stops': len(f.get("itineraries", [{}])[0].get("segments", [])) - 1,
                    'price': float(f.get("price", {}).get("total", 0))
                }
                flight_options.append(option)

            if flight_options:
                cheapest_flight = min(flight_options, key=lambda x: x['price'])
        except Exception as e:
            print(f"Flight API error: {e}")

        try:
            # Fetch Hotels in Sydney
            hotel_resp = requests.get("https://test.api.amadeus.com/v3/shopping/hotel-offers", params={
                "cityCode": "SYD", "checkInDate": "2025-12-15", "checkOutDate": "2025-12-20", "adults": 1
        ...
        if cheapest_flight and cheapest_hotel:
            total_travel_cost = cheapest_flight['price'] + cheapest_hotel['price']
        else:
            total_travel_cost = 0
        # ----------- End: Injected Flight + Hotel Cost Logic -----------
        ...
        translation_cost, base_cost, complexity, accessibility, capacity = estimate_translation_cost(
            similarity_score,
            hub_travel_time,
            num_churches_nearby,
            barriers_info
        ...
        estimated_cost = round(translation_cost + total_travel_cost, -3)
